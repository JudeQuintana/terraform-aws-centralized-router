locals {
  reachability              = { for this in [var.centralized_router.inspect.reachability] : this => this if var.centralized_router.inspect.reachability }
  diagnostics               = { for this in [var.centralized_router.inspect.diagnostics] : this => this if var.centralized_router.inspect.diagnostics }
  provenance                = { for this in [var.centralized_router.inspect.provenance] : this => this if var.centralized_router.inspect.provenance }
  previous_reachability     = var.centralized_router.inspect.policy_diff.previous_reachability != null
  policy_diff               = { for this in [local.previous_reachability] : this => this if local.previous_reachability }
  equivalent_routing_policy = var.centralized_router.inspect.equivalence.equivalent_routing_policy != null
  equivalence               = { for this in [local.equivalent_routing_policy] : this => this if local.equivalent_routing_policy }
  segment_report            = { for this in [var.centralized_router.inspect.segment_report] : this => this if var.centralized_router.inspect.segment_report }
  policy_normalization      = { for this in [var.centralized_router.inspect.policy_normalization] : this => this if var.centralized_router.inspect.policy_normalization }
  connectivity_graph        = { for this in [var.centralized_router.inspect.connectivity_graph] : this => this if var.centralized_router.inspect.connectivity_graph }
  has_assertions            = var.centralized_router.inspect.assertions != null
...
resource "local_file" "this_reachability_simplified" {
  ...
  filename = format("%s/inspect/%s-reachability-simplified.json", path.root, local.centralized_router_full_name)
}
...
resource "local_file" "this_segment_report" {
  ...
  filename = format("%s/inspect/%s-segment-report.json", path.root, local.centralized_router_full_name)
}
...
resource "local_file" "this_policy_normalization" {
  ...
  filename = format("%s/inspect/%s-policy-normalization.json", path.root, local.centralized_router_full_name)
}
...
resource "local_file" "this_connectivity_graph" {
  ...
  filename = format("%s/inspect/%s-connectivity-graph.dot", path.root, local.centralized_router_full_name)
}
